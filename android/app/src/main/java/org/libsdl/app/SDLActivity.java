...
*/
    ...
    // ==== LÖVE embedding (Flutter PlatformView) support ====
    // When embedded, SDL is NOT the launched Activity: it renders into a SurfaceView
    ...
    public static Handler mEmbedCommandHandler = null;

    // Texture (SurfaceProducer) rendering mode: SDL renders into a Flutter-managed
    // texture-backed Surface instead of an on-screen SurfaceView.
    ...
    /** Static equivalent of main(), used by the SDL thread when embedded. */
    public static void runEmbeddedMain() {
        Log.v("SDL", "Running embedded main " + mEmbedMainFunc + " from " + mEmbedMainLib);
        SDLActivity.nativeRunMain(mEmbedMainLib, mEmbedMainFunc, mEmbedArgs);
        Log.v("SDL", "Finished embedded main function");
    }

    /** Command dispatch that works whether or not an SDLActivity instance exists. */
    static boolean sendCommandStatic(int command, Object data) {
        if (mIsEmbedded) {
            // mEmbedCommandHandler is created on the UI thread by LoveHost.start().
            // Do NOT create it here: this runs on the SDL thread which has no Looper.
            if (mEmbedCommandHandler == null) {
                return false;
            }
            Message msg = mEmbedCommandHandler.obtainMessage();
            msg.arg1 = command;
            msg.obj = data;
            return mEmbedCommandHandler.sendMessage(msg);
        }
        return mSingleton != null && mSingleton.sendCommand(command, data);
    }
    ...
    public static void handleNativeState() {
        ...
        if (mNextNativeState == NativeState.RESUMED) {
            boolean surfaceReady = (mSurface != null) ? mSurface.mIsSurfaceReady : mEmbedSurfaceReady;
            if (surfaceReady && (mHasFocus || mHasMultiWindow) && mIsResumedCalled) {
                if (mSDLThread == null) {
    ...
    public static boolean setActivityTitle(String title) {
        ...
        return sendCommandStatic(COMMAND_CHANGE_TITLE, title);
    }
    ...
    public static void setWindowStyle(boolean fullscreen) {
        // Called from SDLMain() thread and can't directly affect the view.
        // When embedded we must NOT touch the host window's fullscreen flags.
        if (mIsEmbedded) {
            return;
        }
        if (mSingleton != null) {
    ...
    public static boolean showTextInput(int input_type, int x, int y, int w, int h) {
        ...
        if (mIsEmbedded) {
            ...
            return mEmbedCommandHandler.post(new ShowTextInputTask(input_type, x, y, w, h));
        }
...
//        }
    ...
    public static Surface getNativeSurface() {
        if (mIsEmbedded) {
            // Texture mode: hand SDL the Flutter-managed texture surface.
            return mEmbedTextureSurface;
        }
...
class SDLMain implements Runnable {
    ...
    public void run() {
        ...
        if (SDLActivity.mIsEmbedded) {
            SDLActivity.runEmbeddedMain();
        } else {
        ...
        if (SDLActivity.mIsEmbedded) {
            // Embedded: don't finish any Activity, just mark the thread done.
