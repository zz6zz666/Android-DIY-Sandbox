...
_AstrBot-Android-App_ 是由「zz6zz666」（以下简称"我们"）开发的开源产品。本《隐私政策》旨在向您说明我们如何收集、使用、存储、共享与保护您的信息，以及您如何行使相关权利。请您在使用前仔细阅读并充分理解本政策的全部内容。
...
- 本地存储与缓存：为提升加载速度与离线可用性，我们可能在本地缓存必要的配置或资源文件（包括但不限于AstrBot框架配置、容器环境设置等），不用于识别您的个人身份。

说明：我们不要求您注册账户；在默认情况下，我们不收集可直接识别您身份的信息（如姓名、证件号码等），除非您在沟通中主动提供。您通过NapCatQQ适配器登录的QQ账号信息由该适配器处理，我们不直接收集和存储您的QQ账号凭证。

## 3. 权限使用说明

为实现特定功能，我们仅在您使用相关功能或获得您授权时，申请相应系统权限：

- 网络权限：用于连接AstrBot服务、更新框架内容、访问WebView仪表盘与错误上报（必要）。
- 存储/媒体访问：用于读写Ubuntu容器环境文件、AstrBot框架资源、配置文件及缓存加速（在功能需要时申请）。
- 通知权限：用于向您推送必要的服务提醒（如容器启动状态、机器人运行通知等，可自主选择开启）。
- 相机权限：仅在您需要扫描NapCatQQ登录二维码时请求，用于完成登录流程，使用后不会留存相关数据。
- 其他敏感权限（如麦克风、位置等）：本应用默认不申请，仅在您主动使用对应扩展功能时请求，且仅用于实现该功能，不会在后台持续获取。
...
- 提供与维护AstrBot服务、Ubuntu容器环境及应用核心功能；
- 进行安全防护、异常监测（如容器运行错误、服务启动失败）、问题定位与性能优化；
- 统计与分析匿名化使用数据，以改进功能与用户体验；
...
- 广告与统计（如适用）：本应用目前不集成广告功能。如未来为改进服务引入统计分析，将向合作方共享去标识化数据与设备广告标识（如AAID/IDFA），并要求合作方遵循最小化原则与安全要求，不得用于识别您的真实身份。您可在系统中重置或限制广告标识。
...
本应用集成了以下第三方服务，其信息处理活动遵循各自的隐私政策：

- AstrBot框架：详情请参考其官方隐私说明（https://docs.astrbot.app/）
- NapCatQQ消息适配器：详情请参考其官方隐私说明（https://napneko.github.io/guide/napcat）
- Code LFA容器环境：详情请参考其官方隐私说明（https://github.com/nightmare-space/code_lfa）

我们会要求第三方服务提供商仅在必要范围内处理您的信息，并对其数据处理行为进行监督。

## 7. 信息的存储与跨境传输

- 存储地点：我们将境内收集的用户个人信息存储在中国境内。AstrBot框架及第三方服务的信息存储遵循其各自规定。
...
附：权限与信息使用清单
- 网络：用于AstrBot服务连接、WebView仪表盘访问、容器环境网络交互
- 存储/媒体访问：读取Ubuntu容器文件、AstrBot框架资源与配置文件、缓存数据